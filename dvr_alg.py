...
def mark_inf_vals(r_node):
    ...
    #r_node.dvr_matrix[row_num][row_num] = 0 # Marks a node's distance to itself as 0
            ...
            for j in range(0, 5):  # Find that change
                if cur_dvr[i][j] != prev_dvr[i][j]:
                    ...
                    for k in range(0, 5):  # And compare that new value to the rest of the values in that row
                        ...
                        if (j != k) and (cur_dvr[i][k] != 0) and (cur_dvr[i][k] != 999) and (cur_dvr[i][j] >= cur_dvr[i][k]):
